...
def non_max_suppression(bboxes , iou_threshold , threshold , box_format = "corners"):
    ...
    # confidence socre가 임계치 보다 높은 경우만 반영
    bboxes = [box for box in bboxes if box[1] > threshold]
    # confidence score 를 기준으로 내림차순 정렬
    ...
    while bboxes:
        ...
        # 선택된 박스와 클래스가 같아야 하고, 비교 대상의 iou가 iou임계치 보다 낮은경우 
        bboxes = [
            box 
            for box in bboxes 
            if box[0]!=chosen_box[0] 
            or intersection_over_union(box[2:], chosen_box[2:] , box_format=box_format) < iou_threshold
        ]
...
def plot_image(image, boxes):
    ...
    # box[2],box[3] = center_x , center_y
    # box[4] ,box[5] = width , height
    for box in boxes:
        box = box[2:]
        assert len(box) == 4, "Got more values "
